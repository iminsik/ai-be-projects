...
class GPUMemoryManager:
    ...
    async def estimate_model_memory(
        self, model_type: str, batch_size: int, sequence_length: int = 512
    ) -> int:
        ...
        logger.info(
            f"Estimated memory for {model_type} (batch={batch_size}): {total_memory}MB"
        )
        return total_memory

    async def can_allocate_memory(
        self, required_memory: int, device_id: int = 0
    ) -> Tuple[bool, str]:
        ...
        async with self.memory_lock:
            ...
            if available_memory >= required_memory:
                return (
                    True,
                    f"Memory available: {available_memory}MB >= {required_memory}MB",
                )
            else:
                return (
                    False,
                    f"Insufficient memory: {available_memory}MB < {required_memory}MB",
                )

    async def allocate_memory(
        self, job_id: str, required_memory: int, device_id: int = 0
    ) -> bool:
        ...
        async with self.memory_lock:
            can_allocate, reason = await self.can_allocate_memory(
    ...
    async def wait_for_memory(
        self, required_memory: int, device_id: int = 0, timeout: int = 300
    ) -> bool:
    ...
    async def optimize_job_parameters(self, job_data: Dict) -> Dict:
        ...
        if optimized_batch_size < current_batch_size:
            ...
            optimized_hyperparameters["gradient_accumulation_steps"] = (
                gradient_accumulation_steps
            )
            logger.info(
                f"Optimized batch size: {current_batch_size} -> {optimized_batch_size}"
            )
            logger.info(
                f"Added gradient accumulation steps: {gradient_accumulation_steps}"
            )
    ...
    async def get_memory_usage_summary(self) -> Dict:
        ...
        async with self.memory_lock:
            total_allocated = sum(
                job["allocated_memory"] for job in self.active_jobs.values()
            )
            pending_requirements = sum(
                job.estimated_memory for job in self.pending_jobs.values()
            )
